...
def obter_nome_commodity(ticker: str) -> str:
    """
    Retorna o nome da commodity a partir do ticker.

    Args:
        ticker (str): O ticker da commodity (ex: 'KC=F' para Café).

    Returns:
        str: O nome da commodity (ex: 'Café Arábica') ou o ticker se não encontrado.
    """
    nomes_comodities = {
...
def buscar_noticias_rss(termo: str, termo_ingles: str, fontes_rss: List[str]) -> List[Dict[str, str]]:
    """
    Busca notícias de fontes RSS.

    Args:
        termo (str): Termo de busca em português.
        termo_ingles (str): Termo de busca em inglês.
        fontes_rss (List[str]): Lista de URLs de feeds RSS.

    Returns:
        List[Dict[str, str]]: Lista de dicionários contendo título e link das notícias.
    """
    noticias = []
    palavras_relevantes = [
...
def buscar_noticias_scraping(termo: str, termo_ingles: str, fontes_scraping: List[str]) -> List[Dict[str, str]]:
    """
    Busca notícias através de web scraping.

    Args:
        termo (str): Termo de busca em português.
        termo_ingles (str): Termo de busca em inglês.
        fontes_scraping (List[str]): Lista de URLs para fazer scraping.
    ...
    for url in fontes_scraping:
        try:
            ...
            if response.status_code != 200:
                logging.warning(f"Erro HTTP {response.status_code} ao acessar {url}")
                continue

            soup = BeautifulSoup(response.text, "html.parser")
            articles = soup.find_all(re.compile("h[2-3]"), class_=re.compile("title|headline"))
...
def buscar_noticias(termo: str, ticker: str) -> List[Dict[str, str]]:
    """
    Busca notícias relevantes para uma commodity, combinando fontes RSS e scraping.

    Args:
        termo (str): Termo de busca em português (ex: 'Café Arábica').
        ticker (str): Ticker da commodity (ex: 'KC=F').
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        rss_future = executor.submit(buscar_noticias_rss, termo, termo_ingles, fontes_rss)
        scraping_future = executor.submit(buscar_noticias_scraping, termo, termo_ingles, fontes_scraping)
