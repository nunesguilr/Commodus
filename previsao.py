...
def plot_predictions(y_test, predicted_prices, scaler, data, ticker, predicted_next):
    ...
    predicted_prices = predicted_prices.reshape(
        -1, 1) if predicted_prices.ndim == 1 else predicted_prices

    y_test_actual = scaler.inverse_transform(
        np.concatenate(
            [y_test, np.zeros((y_test.shape[0], data.shape[1]-1))], axis=1)
    )[:, 0]

    predicted_actual = scaler.inverse_transform(
        np.concatenate([predicted_prices, np.zeros(
            (predicted_prices.shape[0], data.shape[1]-1))], axis=1)
    )[:, 0]
